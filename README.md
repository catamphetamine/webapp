...
сделать ограничение на размер аватара - мегабайтов 10 (проверять на клиенте, а не на сервере; брать из configuration.image_service.file_size_limit).

react-dnd загрузку аватара (при наведении - анимированно показывать синюю рамку, и добавлять класс card--elevated)
