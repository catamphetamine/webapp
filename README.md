...
проверить drag'n'drop загрузку pattern'а

сделать кнопку сверху "сменить вывеску", и загрузка вывески тогда будет не drag'n'drop, а просто по щелчку.

разделить фон на центральный banner и фоновую мозаику (или цвет).
центральный баннер будет занимать всю высоту, и центроваться, а фоновая мозаика (или цвет) будет заполнять остальное пространство.

фоновый шаблон (background pattern).
вывеска (banner).

сделать опцию повторяющегося background-а


consul update container

сделать выбор цвета в правке, который ещё на ходу бы менял его (в движении)

перенести submit в personal info + connect

в профиле сделать раздел links (твиттеры, вконтакты, и т.п.: "I'm on etsy.com, vk.com, ...")

при статусе "онлайн" — писать, что online
...
у пользователя в профиле: подписки, activity, фотографии.
