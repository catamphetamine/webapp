...
при блокировке пользователя - блокировать /validate для всех токенов (redis, база): сначала писать blocked_at, и потом уже делать revoke для всех токенов.

не пропускать sign_in при blocked_at




показывать снек, что пользователь заблокирован, и куда-нибудь перенаправлять (мб на главную).

а может быть просто менять render(), что "пользователь такой-то заблокирован".

можно перенаправлять на профиль пользователя, что там появляется "заблокирован тогда-то тем-то".
...
заблокировать свою учётную запись чтобы можно было без логина

сделать блокировку пользователя по токену (+ причина блокировки, дата, и кем)

в профиле пользователя выводить, если заблокирован (когда, кем, причина)

истечение токена в течение суток (лучше - 7 дней) с удалением из базы

заблокировать пользователя по токену может только он сам
