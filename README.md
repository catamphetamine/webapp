...
/password/check - защитить кулдауном, таким же, как вход (использовать те же самые переменные)
(такая же защита будет на подтверждении пароля при смене емейла, поэтому вообще вынести проверку пароля в функцию check_password, которая будет все эти ограничения проверять и обновлять)
