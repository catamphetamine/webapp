...
переделать нормально, классами на модульной сетке:

  submit
    margin-top: 1em

(проверить этот margin как в случае self, так и в случае не self)
...
перевести текстовый email + strip tags

заблокировать пользователя по токену может только он сам

добавить в шаблон текст о том, что "если это не вы, то возможно, что вашу учётную запись пытаются взломать, свяжитесь с техподдержкой."
