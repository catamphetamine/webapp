...
сбрасывать его только при succeeded.

если "попробуйте позже", то писать, через сколько (для этого написать обратную формулу).

лочить access code после 10-ти попыток

расширить словари

график сгенерированных access codes и график входов

график попыток блокировки пользователя (мб)

ограничить по времени токен блокировки пользователя (7 днями)

запросы на блокировку пользователя может быть как-то троттлить тоже

график попыток входа по access code, по паролю - мониторинг количества неудавшихся входов
...
добавить двухфакторную аутентикацию - по паролю
